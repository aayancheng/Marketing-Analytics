import { Search, SlidersHorizontal, PieChart, Menu, X } from 'lucide-react';

const NAV_ITEMS = [
  { id: 'lookup', label: 'Customer Lookup', icon: Search },
  { id: 'whatif', label: 'CLV Simulator', icon: SlidersHorizontal },
  { id: 'segments', label: 'Portfolio Explorer', icon: PieChart },
];
...
export default function Sidebar({ activeView, onNavigate, open, onToggle }) {
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`fixed left-0 top-0 h-screen w-56 bg-gradient-to-b from-brand-950 to-brand-900 flex flex-col text-white z-40 transition-transform md:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="px-5 py-6">
          ...
          <p className="text-xs text-brand-300 mt-1">Customer Lifetime Value</p>
        </div>
